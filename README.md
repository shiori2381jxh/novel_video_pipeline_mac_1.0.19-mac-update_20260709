...
“专名本地化/改名（任务内记忆）”默认关闭。开启后，洗稿会仍以原文语言输出：中文原文润色为中文，日语原文润色为日语，不会调用翻译流程。文字模型按“洗稿 Skill（可编辑）”重构句式、衔接和表达，同时为人名、地名、家族名、组织名、机构名、种族名生成明显不同的新名称；同一任务的后续批次会继承此前生成的映射，最终正文再由程序统一替换，确保 TTS、字幕、分镜使用同一套名字。

任务目录的 `text_rewrite_replacements.json` 记录语言判定、映射、警告与旧名残留项；`text_rewrite_report.json` 会记录每批字数比例和映射数量。单字或容易成为普通词的名称不会由程序盲目全文替换，会在报告中提示人工复核。模型返回 JSON 不合规、内容明显过短/过长或语言不一致时会自动重试一次；仍失败则整次保留原文，不把半成品送入后续阶段。选择“重做洗稿和配音”会清除任务内映射，按当前 Skill 和设置重新生成。
